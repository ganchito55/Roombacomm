...
public abstract class RoombaComm {
	...
	public short[] getIrFront() {
		...
		irValues[1] = toShort(sensor_bytes[2], sensor_bytes[3]);
		irValues[2] = toShort(sensor_bytes[4], sensor_bytes[5]);
		irValues[3] = toShort(sensor_bytes[6], sensor_bytes[7]);
		irValues[4] = toShort(sensor_bytes[8], sensor_bytes[9]);
		irValues[5] = toShort(sensor_bytes[10], sensor_bytes[11]);
	...
	/**
	 * Compute possible safety fault. Called on every successful
	 * updateSensors(). In normal use, call updateSensors() then check
	 * safetyFault().
	 * 
